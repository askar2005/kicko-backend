...
router.post('/owner/register', async (req, res) => {
    try {
        ...
        const token = jsonwebtoken_1.default.sign({ id: owner.id, email: owner.email, role: 'owner' }, JWT_SECRET, { expiresIn: '7d' });
...
router.post('/owner/login', async (req, res) => {
    try {
        ...
        const isMatch = await bcrypt_1.default.compare(password, owner.password);
        if (!isMatch) {
            return res.status(401).json({ error: 'The email or password you entered is incorrect.' });
        }
        const token = jsonwebtoken_1.default.sign({ id: owner.id, email: owner.email, role: 'owner' }, JWT_SECRET, { expiresIn: '7d' });
...
router.post('/admin/register', async (req, res) => {
    try {
        ...
        const token = jsonwebtoken_1.default.sign({ id: admin.id, email: admin.email, role: 'admin' }, JWT_SECRET, { expiresIn: '7d' });
