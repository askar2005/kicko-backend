...
const calculateBookingAmount = (booking) => {
    const slotLabel = (0, exports.normalizeSlotLabel)(`${booking.startTime} - ${booking.endTime}`);
    let slotPricesMap = {};
    try {
        if (booking.turf.slotPrices) {
            ...
            if (parsed && typeof parsed === 'object' && !Array.isArray(parsed)) {
                slotPricesMap = Object.entries(parsed).reduce((acc, [slot, price]) => {
                    const amount = Number(price);
                    if (Number.isFinite(amount)) {
                        acc[(0, exports.normalizeSlotLabel)(slot)] = amount;
                    }
                    return acc;
                }, {});
            }
